...
class PDFToRawConverter:
    def __init__(self, root):
        ...
        self.output_dir = r"D:\Desktop\项目\MinerU\MinerU输出"

        # 轮询配置（可通过UI调整）
        self.poll_interval_var = tk.IntVar(value=15)    # 默认轮询间隔（秒）
        self.max_attempts_var = tk.IntVar(value=240)    # 默认最大轮询次数
    ...
    def setup_ui(self):
        ...
        # 轮询设置（适用于大文件，可延长间隔与最大次数）
        poll_frame = ttk.Frame(options_frame)
        poll_frame.grid(row=2, column=0, columnspan=2, sticky=(tk.W, tk.E), pady=5)
        ttk.Label(poll_frame, text="轮询间隔(秒):").grid(row=0, column=0, padx=(0, 6))
        self.poll_spin = tk.Spinbox(poll_frame, from_=5, to=3600, increment=5, textvariable=self.poll_interval_var, width=6)
        self.poll_spin.grid(row=0, column=1, padx=(0, 12))
        ttk.Label(poll_frame, text="最大轮询次数:").grid(row=0, column=2, padx=(0, 6))
        self.max_spin = tk.Spinbox(poll_frame, from_=10, to=10000, increment=10, textvariable=self.max_attempts_var, width=8)
        self.max_spin.grid(row=0, column=3)
    ...
    def start_conversion(self):
        ...
        try:
            ...
            if poll_interval < 1 or max_attempts < 1:
                raise ValueError
        except Exception:
            messagebox.showerror("错误", "轮询间隔和最大轮询次数必须是正整数")
            return
    ...
    def poll_task_status(self, task_id):
        ...
        self.log_message(f"开始轮询任务状态，间隔 {poll_interval} 秒，最大 {max_attempts} 次。")
