...
# ========== 在这里修改输入输出路径 ==========
# 示例（请用你自己的路径替换下面的值）：
MARKDOWN_PATH = r"D:\Desktop\项目\MinerU\MinerU输出\2025年中国急性肠系膜上动脉栓塞诊断与治疗急诊专家共识_原始文件\full.md"
IMAGES_DIR = r"D:\Desktop\项目\MinerU\MinerU输出\2025年中国急性肠系膜上动脉栓塞诊断与治疗急诊专家共识_原始文件\images"
OUTPUT_DOCX = r"D:\Desktop\项目\MinerU\MinerU输出\2025年中国急性肠系膜上动脉栓塞诊断与治疗急诊专家共识_原始文件\output.docx"
# 最大图片宽度（英寸）
...
def preprocess_math(md_text):
    """
    将 Markdown 中的 LaTeX 风格数学（$...$ / $$...$$）做简单去语法化处理，
    以及去掉转义的百分号（\% -> %）。
    """
    if not md_text:
        return md_text
    ...
    # 处理显示数学 $$...$$，再处理行内数学 $...$
    md_text = re.sub(r'\$\$(.+?)\$\$', lambda m: replace_inner(m.group(1)), md_text, flags=re.S)
    md_text = re.sub(r'\$(.+?)\$', lambda m: replace_inner(m.group(1)), md_text, flags=re.S)
    return md_text
